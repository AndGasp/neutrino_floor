...
def plot_exposure(exp_list,cs_list,label_list,style_list,save_name):
	return 0




def plot_floor_list(m_list,cs_list,label_list,style_list, save_name, exp_show=True):
	#neu_floor numpy array containing floor with data-type [('mass', float), ('cs', float), ('exp', float)]
	...
	for j in range(len(style_list)):
		plt.plot(m_list[j],cs_list[j],style_list[j],ms=12, label=label_list[j])
...
def asympt(m,a,b,c,d):
	return c + b/(m-d) + a*m

def fit_floor(m,cs):
	
	m_tab = np.logspace(1,4,500)
	
	extrapolator = UnivariateSpline(np.log(m), np.log(cs), k=2 )
	y = extrapolator(np.log(m_tab))
	"""
	popt,pcov = scipy.optimize.curve_fit(asympt,np.log(m),np.log(cs))
	y = asympt(m_tab,popt[0],popt[1],popt[2],popt[3])
	"""
	

	return m_tab,np.exp(y)


def load_floor(npy_file):
	...
	#plt.plot(np.log(m),np.log(cs))
	#plt.show()
	return m,cs,e

"""
file_name = 'floor_Xe_withER_basic_10000.txt'
floor = text_to_numpy(file_name) #convert tet file to array
plot_floor(floor, 'xenon_1000_100.png', exp_show=False) #plot floor
"""