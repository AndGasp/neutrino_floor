...
if mod == "cs": 
	...
	list_info_cs = [n_dicho,n_mc,E_min,E_max,n_bin,neu_arr,AA,ZZ,eff_er,speed_dist,dist]
	...
	if len(exp_tab)>1:
		...
		m_pairs[:,1] = exp_tab[0]*np.ones(n_tasks)
	tasks = []
...
if mod == 'exp': 
	...
	list_info_exp = [n_bin,n_mc,E_min,E_max,n_bin,neu_arr,AA,ZZ,eff_er,speed_dist,dist]
	...
	def multi_task(task_tab,co_args_exp=list_info_exp):
		...
		if cs>1e-40 or cs<1e-52:
			...
			print('###m={} GeV, cs={} cm2, exposure={} Ty'.format(m,cs,exp_floor))
...
if __name__ == '__main__':
	...
	#map single_task to availble Pool processes
...
for k in range(len(m_tab)):
	m,c,ex = multi_task(tasks[k])